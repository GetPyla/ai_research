...
code_agent = Agent('openai:gpt-3.5-turbo', system_prompt="You are a helpful assistant.")
...
async def execute_code(code: str) -> str:
    ...
    Utilise print() pour afficher la sortie.
    Utilise uniquement la bibliothèque standard.
    """
    st.write(f"Exécution du code :\n```python\n{code}\n```")
    
    riza = rizaio.Riza()
    result = await asyncio.to_thread(riza.command.exec, language="python", code=code, http={"allow": [{"host": "*"}]})
    
    if result.exit_code != 0:
        return f"Erreur: {result.stderr}"
    if result.stdout == "":
        return "Le code s'est exécuté sans erreur mais n'a produit aucune sortie."
...
async def run_agent_with_streaming(user_input: str):
    ...
    try:
        async with code_agent.run_stream(
            user_input,
            deps=deps,
...
async def main():
    st.title("AI agent that can run python")
